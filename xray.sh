...
getVersion() {
    ...
    NEW_VER="$(normalizeVersion "$(curl -s "${TAG_URL}" --connect-timeout 10 | jq .[0] | jq -r .tag_name)")"
...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        if [[ -f ~/xray.pem && -f ~/xray.key ]]; then
            ...
            resolve=`curl --http2 -H 'accept: application/dns-json' https://1.1.1.1/dns-query?name=${DOMAIN} | jq -r '.["Answer"][0]["data"]'`
